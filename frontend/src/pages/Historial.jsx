...
export default function Historial() {
  ...
  const [pdfCargando, setPdfCargando]   = useState(null) // { id, plantillaId }
  const LIMIT = 15
  ...
  async function ocultar(r) {
    ...
    try {
      await api.delete(`/requisiciones/${r.id}`, {
        data: { motivo: resultado.value || '' },
      })
      await Swal.fire({ title: 'Ocultada', icon: 'success', timer: 1800, showConfirmButton: false })
      cargar()
    } catch (err) {
      Swal.fire('Error', err.response?.data?.mensaje || 'No se pudo ocultar.', 'error')
    }
  }
  ...
  async function restaurar(r) {
    ...
    try {
      ...
      Swal.fire('Error', err.response?.data?.mensaje || 'Error al restaurar.', 'error')
    }
  }
  ...
  async function guardarEdicion(e) {
    ...
    try {
      ...
      Swal.fire('Error al guardar', err.response?.data?.mensaje || 'Error desconocido.', 'error')
    } finally {
      setGuardando(false)
    }
  }
  ...
  // ── GENERAR PDF ────────────────────────────────────────────────────────────
  async function handlePDF(req, prev, plantillaId) {
    setPdfCargando({ id: req.id, plantillaId })
    try {
      await generarPDF(req, prev, plantillaId)
    } finally {
      setPdfCargando(null)
    }
  }
            ...
            <div style={{ display:'flex', gap:10, alignItems:'center', flex:1, flexWrap:'wrap' }}>
              ...
              {esAdmin && (
                <label style={{
                  ...
                  borderRadius:8, background: verOcultas ? 'var(--dorado-suave)' : 'transparent',
                  transition:'all .2s',
                }}>
        ...
        <div className="modal-overlay" onClick={() => setSeleccionada(null)}>
          <div className="modal" style={{ maxWidth:580 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              {/* Aviso de la ruta de plantillas */}
              <div style={{
                fontSize:'.75rem', color:'var(--texto-claro)',
                marginBottom:14, padding:'8px 12px',
                background:'var(--verde-suave)', borderRadius:8,
                display:'flex', alignItems:'flex-start', gap:8,
              }}>
                <IcoDoc />
                <span>
                  Selecciona un documento para generar. Las plantillas deben estar en{' '}
                  <code style={{ background:'rgba(0,0,0,.07)', padding:'0 4px', borderRadius:3 }}>
                    frontend/public/plantillas/
                  </code>
                </span>
              </div>
                  ...
                  return (
                    <div key={p.id} className="doc-row">
                      ...
                      <div className="doc-acciones">
                        <button
                          className="btn-icon verde"
                          title="Previsualizar"
                          disabled={!!pdfCargando}
                          onClick={() => handlePDF(seleccionada, true, p.id)}
                        >
                          <IcoEye />
                        </button>
                        <button
                          className="btn-icon gris"
                          title="Descargar"
                          disabled={!!pdfCargando}
                          onClick={() => handlePDF(seleccionada, false, p.id)}
                        >
                          <IcoDl />
                        </button>
                      </div>
              ...
              <div className="modal-body">
                <div className="form-grid">
                  <div className="form-group full">
                    ...
                    <textarea rows={3} value={editando.concepto || ''}
                  ...
                  <div className="form-group">
                    ...
                    <input type="number" step="0.01" min="0.01"
                      ...
                      onChange={e => setEditando(f => ({ ...f, monto: e.target.value }))}
                      required />
                  </div>
                  <div className="form-group">
