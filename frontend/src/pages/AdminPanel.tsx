...
import { Requisicion, UsuarioAdmin, RegistroAuditoria, FormularioUsuario, Rol } from '../types';
...
type TabId = 'ocultas' | 'usuarios' | 'auditoria' | 'purgar';
...
const NUEVO_USUARIO: FormularioUsuario = { nombre: '', username: '', password: '', rol: 'usuario' };
...
export default function AdminPanel() {
  ...
  const [tab,        setTab]        = useState<TabId>('ocultas');
  const [ocultas,    setOcultas]    = useState<Requisicion[]>([]);
  const [usuarios,   setUsuarios]   = useState<UsuarioAdmin[]>([]);
  const [auditoria,  setAuditoria]  = useState<RegistroAuditoria[]>([]);
  const [loading,    setLoading]    = useState(false);
  const [newUser,    setNewUser]    = useState<FormularioUsuario>(NUEVO_USUARIO);
  ...
  async function cargarOcultas() {
    setLoading(true);
    try { const { data } = await api.get<Requisicion[]>('/admin/requisiciones-ocultas'); setOcultas(data); }
    catch { setOcultas([]); }
  ...
  async function cargarUsuarios() {
    setLoading(true);
    try { const { data } = await api.get<UsuarioAdmin[]>('/admin/usuarios'); setUsuarios(data); }
    catch { setUsuarios([]); }
    finally { setLoading(false); }
  }
  ...
  async function cargarAuditoria() {
    setLoading(true);
    try {
      const { data } = await api.get<{ datos: RegistroAuditoria[] }>('/admin/auditoria');
      setAuditoria(data.datos ?? []);
    }
    catch { setAuditoria([]); }
    finally { setLoading(false); }
  }
  ...
  async function eliminarDefinitivamente(r: Requisicion) {
    const conf = await Swal.fire({
      title:             'Eliminar definitivamente',
      html:              `<p>La requisición <b>${r.folio}</b> será eliminada de forma <b>permanente</b>.</p>
                         <p style="margin-top:10px;color:#b91c1c;font-size:.85rem">Esta acción <b>NO SE PUEDE DESHACER</b>.</p>
                         <p style="margin-top:8px;font-size:.8rem;color:#666">Para confirmar, escribe el folio: <b>${r.folio}</b></p>`,
      input:             'text',
    ...
    try {
      ...
      Swal.fire({ title: 'Error', text: msg ?? 'No se pudo eliminar.', icon: 'error', confirmButtonColor: '#1a3a2a' });
    } finally {
      setEliminando(null);
    }
  }
  ...
  // ── GESTIÓN DE USUARIOS ───────────────────────────────────────
  async function toggleUsuario(u: UsuarioAdmin) {
  ...
  async function crearUsuario(e: FormEvent) {
    ...
    try {
      ...
      setNewUser(NUEVO_USUARIO);
      setShowForm(false);
      cargarUsuarios();
    } catch (err) {
      const msg = (err as { response?: { data?: { mensaje?: string } } }).response?.data?.mensaje;
      Swal.fire({ title: 'Error', text: msg ?? 'Error.', icon: 'error', confirmButtonColor: '#1a3a2a' });
    }
  }
  ...
  async function handlePurgar() {
    ...
    const conf = await Swal.fire({
      ...
      html:              `<p>Estás a punto de <b>eliminar definitivamente</b> todas las requisiciones ${anioTexto}.</p>
                         <p style="margin-top:10px;color:#b91c1c;font-size:.85rem">Esta acción <b>NO SE PUEDE DESHACER</b>.</p>
                         <p style="margin-top:8px;font-size:.8rem;color:#666">Para confirmar, escribe: <b>PURGAR</b></p>`,
      input:             'text',
    ...
    try {
      ...
      const { data } = await api.delete<{ afectadas: number }>('/admin/purgar', { params });
      Swal.fire({
      ...
      const msg = (err as { response?: { data?: { mensaje?: string } } }).response?.data?.mensaje;
      Swal.fire({ title: 'Error', text: msg ?? 'No se pudo purgar.', icon: 'error', confirmButtonColor: '#1a3a2a' });
    } finally {
      setPurgando(false);
    }
  }
  ...
  const accionColor = (a: string): string =>
    a === 'ocultar'   ? 'var(--rojo)'       :
  ...
  const rolBadgeStyle = (rol: Rol): React.CSSProperties => ({
      ...
      <div className="page-body fade-in">
        ...
        {tab === 'ocultas' && (
          <div className="card">
            ...
            {esAdmin && ocultas.length > 0 && (
              <div style={{ margin: '0 24px 4px', padding: '10px 14px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, fontSize: '.78rem', color: '#991b1b', display: 'flex', alignItems: 'center', gap: 8 }}>
                <IcoAlertTri />
                <span><b>Eliminar definitivamente</b> borra el registro de forma permanente e irreversible. Usa <b>Restaurar</b> si solo quieres que vuelva a ser visible.</span>
              </div>
            )}
            ...
            {loading ? (
              ...
              <div className="table-wrap">
                <table>
                  <thead>
                    <tr>
                      <th>Folio</th><th>Beneficiario</th><th>Monto</th>
                      <th>Ocultada por</th><th>Motivo</th><th>Fecha</th>
                  ...
                  <tbody>
                    {ocultas.map(r => (
                      <tr key={r.id} style={{ background: '#fffbeb' }}>
                        ...
                        <td style={{ fontSize: '.78rem', color: 'var(--texto-claro)', maxWidth: 160 }}>{r.oculta_motivo ?? '—'}</td>
        ...
        {tab === 'usuarios' && esAdmin && (
          <div className="card">
            ...
            {showForm && (
              <form onSubmit={crearUsuario} style={{ padding: '20px 24px', borderBottom: '1px solid var(--gris-20)', background: 'var(--gris-10)' }}>
                <div className="form-grid" style={{ maxWidth: 640 }}>
                  <div className="form-group">
                    <label>Nombre completo</label>
                    <input required value={newUser.nombre} onChange={e => setNewUser(f => ({ ...f, nombre: e.target.value }))} />
                  </div>
                  <div className="form-group">
                    <label>Usuario (login)</label>
                    <input required value={newUser.username} onChange={e => setNewUser(f => ({ ...f, username: e.target.value.toLowerCase() }))} />
                  </div>
                  <div className="form-group">
                    <label>Contraseña</label>
                    <input type="password" required minLength={6} value={newUser.password} onChange={e => setNewUser(f => ({ ...f, password: e.target.value }))} />
                  </div>
                  <div className="form-group">
                    <label>Rol</label>
                    <select value={newUser.rol} onChange={e => setNewUser(f => ({ ...f, rol: e.target.value as Rol }))}>
                  ...
                  <tbody>
                    {usuarios.map(u => (
                      <tr key={u.id}>
                        ...
                        <td>
                          <span style={{ fontSize: '.75rem', display: 'inline-flex', alignItems: 'center', gap: 4, color: u.activo ? 'var(--verde-claro)' : 'var(--rojo)' }}>
                            <span style={{ width: 7, height: 7, borderRadius: '50%', background: u.activo ? 'var(--verde-claro)' : 'var(--rojo)', display: 'inline-block' }} />
                        ...
                        <td>
                          <div className="acciones-cell" style={{ justifyContent: 'center' }}>
                            <button className="btn btn-secondary" style={{ fontSize: '.72rem', padding: '4px 10px' }} onClick={() => resetPassword(u)}>
                              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><IcoKey />Reset</span>
                            </button>
                            <button className={`btn ${u.activo ? 'btn-danger' : 'btn-secondary'}`} style={{ fontSize: '.72rem', padding: '4px 10px' }} onClick={() => toggleUsuario(u)}>
                              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  <tbody>
                    {auditoria.map(a => (
                      <tr key={a.id}>
                        ...
                        <td>
                          <span style={{ fontSize: '.72rem', fontWeight: 600, textTransform: 'uppercase', color: accionColor(a.accion) }}>
                            {a.accion}
                          </span>
                        </td>
            ...
            <div className="card-body">
              <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '16px 20px', marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 700, color: '#7f1d1d', marginBottom: 6 }}>Zona de peligro — Acción irreversible</div>
              ...
              <div style={{ maxWidth: 480 }}>
                <div className="form-group" style={{ marginBottom: 20 }}>
                  <label style={{ fontWeight: 600, marginBottom: 8, display: 'block' }}>¿Qué datos purgar?</label>
                  <select value={anioFiltro} onChange={e => setAnioFiltro(e.target.value)} style={{ width: '100%' }}>
