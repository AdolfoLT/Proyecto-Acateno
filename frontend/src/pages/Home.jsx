...
// ── Hojas auxiliares/catálogos a ignorar ─────────────────────
...
// Mapeo de nombres de columna → campo interno
const COL_MAP = {
  'FECHA':                     'fecha',
...
export default function Home() {
  ...
  const [stats, setStats]                     = useState({ total: 0, monto: 0, hoy: 0 })
  ...
  useEffect(() => {
    api.get('/requisiciones/catalogos/areas').then(r => setAreas(r.data)).catch(() => {})
    api.get('/requisiciones/catalogos/clasificaciones').then(r => setClasificaciones(r.data)).catch(() => {})
    cargarStats()
  }, [])

  async function cargarStats() {
    try {
      const r = await api.get('/requisiciones?limit=1000')
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true)
    try {
      const { data } = await api.post('/requisiciones', form)
      Swal.fire({
        title: 'Guardado',
        html: `Requisición <b>${data.folio}</b> creada con éxito.`,
        icon: 'success', confirmButtonColor: '#1a3a2a',
      })
      setForm(vacío)
      cargarStats()
    } catch (err) {
      Swal.fire({ title: 'Error', text: err.response?.data?.mensaje || 'No se pudo guardar.', icon: 'error', confirmButtonColor: '#1a3a2a' })
    } finally {
      setSaving(false)
    }
  }
  ...
  async function procesarExcel(file) {
    ...
    for (const nombreHoja of wb.SheetNames) {
      ...
      for (let i = headerIdx + 1; i < raw.length; i++) {
        ...
        // Monto: prioridad cheque > transferencia > efectivo > total
        let monto = parsearMonto(get(fila, 'monto_cheque'))
        let formaPago = 'CHEQUE'
        ...
        // Si la hoja tiene columna forma_pago explícita, usarla
        if (colIdx['forma_pago'] !== undefined) {
          const fp = norm(get(fila, 'forma_pago'))
        ...
        try {
          ...
          errores.push(`Fila ${i + 1} — ${beneficiario}: ${err.response?.data?.mensaje || err.message}`)
        }
      }
    }
    ...
    if (totalFilas === 0) {
      Swal.fire({
        ...
        html: 'No se encontraron filas con monto válido en el archivo.',
        icon: 'warning', confirmButtonColor: '#1a3a2a',
      })
      return
    }
    ...
    Swal.fire({
      title: 'Importación completada', html,
      icon: totalOk > 0 ? 'success' : 'warning', confirmButtonColor: '#1a3a2a',
    })
    cargarStats()
  }
        ...
        <div className="card">
          <div className="card-header">
            ...
            <div className="tabs" style={{ marginBottom: 0 }}>
              {[
                { id: 'manual', label: 'Manual',         icon: <IcoEdit /> },
                { id: 'excel',  label: 'Importar Excel', icon: <IcoTable /> },
              ].map(m => (
                <button key={m.id} className={`tab-btn ${modo === m.id ? 'activo' : ''}`}
          ...
          <div className="card-body">
            {modo === 'manual' ? (
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    <select value={form.area_id} onChange={e => {
                      ...
                      set('area_id', e.target.value); set('area_nombre', op?.nombre || '')
                    }}>
                  ...
                  <div className="form-group full">
                    ...
                    <textarea placeholder="Describe el concepto del gasto…"
                  ...
                  <div className="form-group">
                    ...
                    <input placeholder="Nombre completo o razón social"
                      value={form.proveedor} onChange={e => set('proveedor', e.target.value)} required />
                  </div>

                  <div className="form-group">
                    <label>RFC</label>
                    <input placeholder="RFC del beneficiario"
                      value={form.rfc} onChange={e => set('rfc', e.target.value.toUpperCase())} maxLength={13} />
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <input placeholder="Opcional" value={form.no_factura}
                      onChange={e => set('no_factura', e.target.value)} />
                  </div>

                  <div className="form-group">
                    <label>No. de Contrato</label>
                    <input placeholder="Opcional" value={form.no_contrato}
                      onChange={e => set('no_contrato', e.target.value)} />
                  </div>

                  <div className="form-group">
                    <label>Cuenta Bancaria</label>
                    <input placeholder="Opcional" value={form.cuenta_bancaria}
              ...
              <div
                ...
                onDrop={e => { e.preventDefault(); setDrag(false); procesarExcel(e.dataTransfer.files[0]) }}
